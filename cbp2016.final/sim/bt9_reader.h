...
namespace bt9 {
    ...
    private:
        ...
        int openBT9TraceFile_(const std::string & name) {
            ...
            char cmd_c[256];
            char trnm_cmd[256];
            memset((char *) cmd_c,  0, 256);
            memset((char *) trnm_cmd,  0, 256);

            auto gzip_suffix_pos = name.find(".gz");
            if (gzip_suffix_pos != std::string::npos) 
            {
                int pos = name.find_last_of(".");
                sprintf(cmd_c, "\"C:\\Program Files\\7-Zip\\7z.exe\" e %s ", name.c_str());
                system(cmd_c);
                name_copy.erase(pos, std::numeric_limits<std::string::size_type>::max());
            }
            
            FILE * pipe = fopen(name_copy.c_str(), "r");
