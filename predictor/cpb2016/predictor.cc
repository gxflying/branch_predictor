...
bool PREDICTOR::get_prediction(const branch_record_c1 *br, void * p)
{
  unsigned long long PC = br->instruction_addr;
  unsigned long long branchTarget = br->branch_target;
  bool branchTaken = br->taken;
  bool predDir = false;
  OpType opType;  
  ...
  {
    TrackOtherInst(PC, opType, branchTaken, branchTarget);
  }
}
void PREDICTOR::update_predictor(const branch_record_c1 *br, void * p, bool taken)
{
  unsigned long long PC = br->instruction_addr;
  unsigned long long branchTarget = br->branch_target;
  bool branchTaken = br->taken;
  ...
  if (br->is_conditional)
  {
    UpdatePredictor(PC, t, branchTaken, predDir, branchTarget);
  }
}